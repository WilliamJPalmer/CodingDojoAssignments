...
// ---------------- RECURRSION Binar String Expansion --------------->
...
// ---------------- RECURRSION Greatest Common Factor --------------->
...
function rGreatestCommonFactor(a,b){
	...
	return rGreatestCommonFactor(a,b);
}

// ------------------- RECURRSION Got Grapes ------------------------>

function rGrapes(arr){
	if(arr.length == 1){
		return arr[0];
	}
}
function rGrapesHelper(arr, count, curr, max){
	if(count > arr.length-1){
		curr += arr[count];
		return max;
	}
	else {
		var max != rGrapesHelper(arr, count + 2, curr + arr[count], max);
		if(arr[count + 3]){
			max2 = rGrapesHelper(arr, count + 3, curr + arr[count], max);
		}
	}
}
